...
int cades_sign_with_options(const char *data, int data_len,
  ...
  while ((pCertContext = CertEnumCertificatesInStore(hStore, pCertContext)) !=
    ...
    if (CertGetCertificateContextProperty(pCertContext,
                                          CERT_KEY_PROV_INFO_PROP_ID, NULL,
                                          &dwKeyProvInfoSize)) {
      pKeyInfo = (CRYPT_KEY_PROV_INFO *)malloc(dwKeyProvInfoSize);
      if (pKeyInfo && CertGetCertificateContextProperty(
                          pCertContext, CERT_KEY_PROV_INFO_PROP_ID, pKeyInfo,
                          &dwKeyProvInfoSize)) {
        FILETIME ftSystemTime;
        GetSystemTimeAsFileTime(&ftSystemTime);
        if (CertVerifyTimeValidity(&ftSystemTime, pCertContext->pCertInfo) ==
            0) {
  ...
  if (!CryptSignMessage(&signPara, FALSE, 1, pbToBeSigned, cbToBeSigned, NULL,
                        &pSignedBlob->cbData)) {
  ...
  if (!CryptSignMessage(&signPara, FALSE, 1, pbToBeSigned, cbToBeSigned,
                        pSignedBlob->pbData, &pSignedBlob->cbData)) {
    DWORD err = GetLastError();
...
int cades_sign_simple(const char *data, int data_len, unsigned char **out_sig,
                      int *out_len) {
  return cades_sign_with_options(data, data_len, "MY", "1.3.14.3.2.26", out_sig,
                                 out_len);
}
...
int sign_from_example(unsigned char *data, int data_size,
                      unsigned char **signed_message, int *out_len) {
  CRYPT_SIGN_MESSAGE_PARA signPara = {sizeof(signPara)};
  signPara.dwMsgEncodingType = X509_ASN_ENCODING | PKCS_7_ASN_ENCODING;
  signPara.pSigningCert = NULL; // NULL to get window for choosing cert
  ...
  *signed_message = malloc(pSignedMessage->cbData);
  if (!*signed_message) {
    free(pSignedMessage->pbData);
    free(pSignedMessage);
    return -2;
  }

  memcpy(*signed_message, pSignedMessage->pbData, pSignedMessage->cbData);
  *out_len = pSignedMessage->cbData;
